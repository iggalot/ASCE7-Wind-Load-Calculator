...
{
    public class WallAreaCalculator_GableEnd_CC_ASCE7_22 : AreaCalculator_CC_ASCE7_22_Base
    {
        public WallAreaCalculator_GableEnd_CC_ASCE7_22(BuildingData bldg_data, string note_string="")
        {
        ...
        public override void ComputeEffectiveWindAreas()
        {
            ...
            Point C = new Point(buildingData.BuildingLength, buildingData.BuildingHeight);
            Point D = new Point(0, buildingData.BuildingHeight);
            Point E = new Point(length, buildingData.BuildingHeight + Math.Tan(buildingData.RoofPitch * Math.PI / 180.0) * length / 2.0);

            Point p1, p2, p3, p4, p5, p6;
            if (length > 2 * CritDim_a)
            {
                var ht_at_roof = buildingData.BuildingHeight + Math.Tan(buildingData.RoofPitch * Math.PI / 180.0) * CritDim_a;
